...
def process_prospects(year):
    ...
    ROUND(
    ( 5*b.avg_FV +
    0.25*IFNULL(COALESCE(b.fg_pit_OFP_present, b.fg_hit_OFP_present), 0) + 
    1.25*IFNULL(COALESCE(b.fg_pit_OFP_future, b.fg_hit_OFP_future), 0) + 
    0.50*IFNULL(COALESCE(b.mlb_pit_OFP, b.mlb_hit_OFP), 0) 
    ) /
    (5 + 
    0.25*IFNULL((COALESCE(b.fg_pit_OFP_present, b.fg_hit_OFP_present)/COALESCE(b.fg_pit_OFP_present, b.fg_hit_OFP_present)), 0) + 
    1.25*IFNULL((COALESCE(b.fg_pit_OFP_future, b.fg_hit_OFP_future)/COALESCE(b.fg_pit_OFP_future, b.fg_hit_OFP_future)), 0) + 
    0.50*IFNULL((COALESCE(b.mlb_pit_OFP, b.mlb_hit_OFP)/COALESCE(b.mlb_pit_OFP, b.mlb_hit_OFP)), 0) 
    )
    + ((b.sleeper + b.breakout + b.upside + b.personal_traits)/4)
    , 2) AS superAdj_FV,
    ROUND(
    ( 5*b.avg_FV +
    0.25*IFNULL(COALESCE(b.fg_pit_OFP_present, b.fg_hit_OFP_present), 0) + 
    ...
    FROM(
        SELECT
        ROUND( 
        ( 4*IFNULL(fg_FV,0) + 3*IFNULL(mi_FV,0) + 2*IFNULL((mlbp_FV-5),0) + 3*IFNULL(fgd_FV,0) + 1*IFNULL((mlbd_FV-5),0) ) 
        /
        ( 4*IFNULL(fg_FV/fg_FV,0) + 3*IFNULL(mi_FV/mi_FV,0) + 2*IFNULL(mlbp_FV/mlbp_FV,0) + 3*IFNULL(fgd_FV/fgd_FV,0) + 1*IFNULL(mlbd_FV/mlbd_FV,0) ) 
        , 2) AS "avg_FV",
        ROUND(
        (0.25*IFNULL(COALESCE(a.fg_pit_OFP_present, a.fg_hit_OFP_present), 0) + 
        2.75*IFNULL(COALESCE(a.fg_pit_OFP_future, a.fg_hit_OFP_future), 0) + 
        1.00*IFNULL(COALESCE(a.mlb_pit_OFP, a.mlb_hit_OFP), 0) 
        ) /
        (
        0.25*IFNULL((COALESCE(a.fg_pit_OFP_present, a.fg_hit_OFP_present)/COALESCE(a.fg_pit_OFP_present, a.fg_hit_OFP_present)), 0) + 
        2.75*IFNULL((COALESCE(a.fg_pit_OFP_future, a.fg_hit_OFP_future)/COALESCE(a.fg_pit_OFP_future, a.fg_hit_OFP_future)), 0) + 
        1.00*IFNULL((COALESCE(a.mlb_pit_OFP, a.mlb_hit_OFP)/COALESCE(a.mlb_pit_OFP, a.mlb_hit_OFP)), 0) 
        )
        , 2) AS ofp_FV, a.*
        FROM(
            SELECT 
...
def update_tables(year):
    qry = """
    # create semi-temporary master current table
    SET @cur_year = %s;
    ...
    AND (!(cr.player_name = "Will Smith" AND cr.expires = 2021) OR !(cr.player_name = "Luis Garcia" AND cr.expires = 2019)OR cr.player_name IS NULL)
